...
TEXT p256SubInternalFork<>(SB),NOSPLIT,$0
	...
	RET
/* ---------------------------------------*/
TEXT p256SqrInternalFork<>(SB),NOSPLIT,$0
	// x[1:] * x[0]
	MUL	x0, x1, acc1
	UMULH	x0, x1, acc2
	...
	RET
/* ---------------------------------------*/
TEXT p256MulInternalFork<>(SB),NOSPLIT,$0
	// y[0] * x
	MUL	y0, x0, acc0
	UMULH	y0, x0, acc1
...
TEXT ·p256PointAddAffineAsm(SB),0,$264-48
	...
	LDy(z1sqr)
	CALL	p256MulInternalFork<>(SB)    // z1 ^ 3
...
TEXT ·p256PointAddAsm(SB),0,$392-32
	...
	// Begin point add
	LDx(z2in)
	CALL	p256SqrInternalFork<>(SB)    // z2^2
	STy(z2sqr)

	CALL	p256MulInternalFork<>(SB)    // z2^3
	...
	LDx(z1in)
	CALL	p256SqrInternalFork<>(SB)    // z1^2
	STy(z1sqr)
